...
def process_data(spark, covid_data, output_data):
    ...
    :return:
    '''
    covid19_data = os.path.join(covid_data, 'uscounty/*.csv')
    df = spark.read.csv(covid19_data, header=True)
    print('uscounty_count = ', df.count())
    df = df.withColumn('datatime', F.to_date(F.col('date'))).drop('date')
    df.write.partitionBy(['county', 'state']).parquet(os.path.join(output_data, 'uscounty'), 'overwrite')
